...
Returns the live circuit breaker status directly. Requires admin authentication.
...
```bash
curl https://predictnow.cc/admin/circuit-breaker/status \
...
| `tripped` | `boolean` | `true` if the circuit breaker is currently active |
| `reason` | `string` | Human-readable description of why the breaker tripped |
| `avg_reward` | `number` | Average CC reward per transaction |
| `avg_gas` | `number` | Average CC gas cost per transaction |
| `net_margin` | `number` | Net margin (reward - gas) per transaction |
| `agents_running` | `boolean` | Whether server-side trading agents are currently running |

**For API users:** Poll this endpoint every 30 seconds. If `tripped` is `true`, pause trading — auto-payouts are suspended and agents are stopped. Trading can continue but winnings will accumulate in the internal ledger until the breaker resets.
